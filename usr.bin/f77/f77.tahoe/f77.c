...
#ifndef lint
static char sccsid[] = "@(#)f77.c	5.5 (Berkeley) 04/12/91";
#endif /* not lint */
...
loadargs[1] = "-X";
loadargs[2] = "-u";
#if HERE==PDP11 || HERE==VAX || HERE==TAHOE
	loadargs[3] = "_MAIN_";
#endif
#if HERE == INTERDATA
	loadargs[3] = "main";
		...
		case 'i':
			...
			*f2flagp++ = '-';
			while(*f2flagp++ = *s++)
				;
			*f2flagp = ' ';
			if(strcmp(pass2name, PASS2NAME) == 0)
				pass2name = PASS2INAME;
			goto endfor;
#else
...
#if TARGET==PDP11 || TARGET==VAX || TARGET==TAHOE
#	ifdef PASS2OPT
	if(optimflag)
		{
#if TARGET==TAHOE
		sprintf(buff, "%s -f %s %s",
#else
		sprintf(buff, "%s %s %s",
#endif
		 pass2opt, asmpass2, optzfname);
		if( sys(buff) )
			rmf(optzfname);
		else
...
#if TARGET!=INTERDATA && TARGET!=PDP11 && TARGET!=VAX && TARGET!=TAHOE
...
#if HERE==PDP11 || HERE==INTERDATA || HERE==VAX || HERE==TAHOE
	if( (waitpid = fork()) == 0)
		{
		enbint(SIG_DFL);
		execv(ldname, v0);
