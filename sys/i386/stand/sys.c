	...
	int fdesc;
{
	...
	if (io->i_cc <= 0) {
		if ((io->i_flgs & F_FILE) != 0) {
			...
			io->i_bn = io->i_offset / DEV_IOSIZE *
				(DEV_IOSIZE/DEV_BSIZE);
			off = io->i_offset % DEV_IOSIZE;
			size = DEV_IOSIZE;
		}
		io->i_ma = io->i_buf;
		io->i_cc = size;
...
/*#ifndef SMALL*/
	if ((io->i_flgs & F_FILE) == 0) {
		...
		if (io->i_offset % 512 || count % 512) {
			...
			io->i_cc = count;
			io->i_ma = buf;
			io->i_bn = io->i_boff + (io->i_offset / DEV_BSIZE);
			i = devread(io);
			io->i_offset += count;
			if (i < 0)
				errno = io->i_error;
...
# define isdigit(n)	((n>='0') && (n<='9'))
	...
	gotdev:
		...
		file->i_ino.i_dev = makedev(dp-devsw, i);
	}
	file->i_boff = 0;
	devopen(file);
	opendev = file->i_ino.i_dev;
	if (cp != str && *cp == '\0') {
		file->i_flgs |= how+1;
		file->i_cc = 0;
		file->i_offset = 0;
	...
	file->i_fs = *(struct fs *)(file->i_buf);
	if ((i = find(cp, file)) == 0) {
		file->i_flgs = 0;
		errno = ESRCH;
		return (-1);
